...
class CharacteristicController{
    ...
    async update(req, res, next){
        ...
        if(!parameterId && value){
            characteristic = await Characteristic.update({value}, {where:{id}});
        }
        if(!value && parameterId){
            characteristic = await Characteristic.update({parameterId}, {where:{id}});
        }
        if(value && parameterId){
            characteristic = await Characteristic.update({value, parameterId}, {where:{id}});
        }
        return res.json(characteristic);
    }
